...
public class AndroidInputManager implements InputManager, SensorEventListener {
    ...
    public Sensor getSensorById(int type, int id) {
        ...
        if (this.mSensorIdMap.containsKey(Integer.valueOf(id))) {
            return this.mSensorIdMap.get(Integer.valueOf(id));
        }
        return null;
    }
    ...
    @Override // com.transmension.mobile.InputManager
    @SuppressLint({"NewApi"})
    public List<InputManager.MotionRange> getDeviceMotionRanges(int id) {
        InputDevice device;
        List<InputDevice.MotionRange> ranges;
        List<InputManager.MotionRange> result = null;
        if (Build.VERSION.SDK_INT >= 12 && (device = InputDevice.getDevice(id)) != null && (ranges = device.getMotionRanges()) != null) {
            result = new ArrayList<>();
            for (InputDevice.MotionRange range : ranges) {
                InputManager.MotionRange mr = new InputManager.MotionRange();
    ...
    public boolean hasSensor(int type) {
        return (this.mSensorManager == null || this.mSensorManager.getDefaultSensor(type) == null) ? false : true;
    }

    @Override // com.transmension.mobile.InputManager
    public boolean startSensor(int type, int id) {
        int key;
        if (this.mSensorMap.get(Integer.valueOf(id)) != null) {
            return true;
        }
        ...
        if (sensor != null && (key = getSensorId(sensor)) >= 0 && this.mSensorManager.registerListener(this, sensor, 1)) {
            this.mSensorMap.put(Integer.valueOf(key), sensor);
            return true;
        }
        return false;
    }
    ...
    public boolean stopSensor(int type, int id) {
        ...
        if (sensor != null && (key = getSensorId(sensor)) >= 0) {
            ...
            this.mSensorMap.remove(Integer.valueOf(key));
            return true;
        }
        return false;
    }
    ...
    public void onMotionEvent(MotionEvent event) {
        if (this.mActivity != null) {
            if ((event.getSource() & 16) != 0) {
                NativeInputManager.onJoystickEventNative(this.mActivity.mNativeHandle, this, InputManager.JoystickEvent.translate(event));
            } else if ((event.getSource() & 4) == 0) {
                NativeInputManager.onTouchEventNative(this.mActivity.mNativeHandle, this, InputManager.PointerEvent.translate(event));
            }
        }
    }
    ...
    public void onSensorChanged(SensorEvent event) {
        if (this.mActivity != null) {
            InputManager.SensorInputEvent sensorEvent = InputManager.SensorInputEvent.translate(event, getSensorId(event.sensor));
